...
; 功能：一键切换系统代理模式 / TUN 模式，验证/重载配置，开机自启
...
TrayMenu.Add("系统代理模式", (*) => ToggleSystemProxyMode())
TrayMenu.Add("TUN模式", (*) => ToggleTunMode())
TrayMenu.Add()
TrayMenu.Add("🔁 重载配置", (*) => ReloadConfig())
TrayMenu.Add("ℹ️ 验证配置", (*) => (TestConfig() ? _ShowTip("验证通过", "配置文件正确") : ""))
TrayMenu.Add()
TrayMenu.Add("✅ 开机自启", (*) => ToggleAutoStart())
TrayMenu.Add()
TrayMenu.Add("❌ 退出", (*) => ExitApp())

TrayMenu.Default := "系统代理模式"

SetTimer(UpdateMenuChecks, 1000)

Global SysProxyMenuName := "系统代理模式"
Global TunMenuName := "TUN模式"

UpdateMenuChecks() {
    Global SysProxyMenuName, TunMenuName

    Try TrayMenu.Check("✅ 开机自启", IsAutoStartEnabled())

    sysNew := "系统代理模式 (" (IsSystemProxyMode() ? "已开启" : "已关闭") ")"
    if (SysProxyMenuName != sysNew) {
        Try TrayMenu.Rename(SysProxyMenuName, sysNew)
        SysProxyMenuName := sysNew
    }

    tunNew := "TUN模式 (" (IsTunMode() ? "已开启" : "已关闭") ")"
